...
def main(numMonitors, kCrash, kRound, formulaFileName, traceFileName, ):
    numMsg = 0;
    msgSize = 0;
    numRnds = 0;
    # crashArr = [0] * 100

    states, transitions, S, AP, lambdaq, start = readFile(formulaFileName)
    trace = readTrace(traceFileName)
...
if __name__ == "__main__":
    if len(sys.argv) == 6:
        ...
        print("<int1> <int2> <int3> <filename1> <filename2> \t A system with <int1> number of monitors with <int2> number of maximum crashes, communicating after every <int3> rounds, "
              "<filename1> formula file, <filename2> trace file")
